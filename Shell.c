...
void eval(char *cmdline) 
{
	...
	if (f == 0){
        ...
        if (isbg == 0){
		addjob( jobs, f, FG, cmdline);
		sigprocmask (SIG_UNBLOCK, &sig, NULL); 
		waitfg(f);
        } else {
		addjob( jobs, f, BG, cmdline);
		sigprocmask (SIG_UNBLOCK, &sig, NULL);
		printf ( "%d %d %s", pid2jid(f), f, cmdline);
	}
    }
    return;
}
...
int builtin_cmd(char **argv) 
{
	...
	else if (strcmp(argv[0], "jobs") == 0)		{listjobs(jobs); return 1;}

	else if (strcmp(argv[0], "fg") == 0 || strcmp(argv[0], "bg") == 0)		{do_bgfg(argv); return 1;}     
...
void waitfg(pid_t pid)
{
	...
	struct job_t *njob = getjobpid(jobs, pid);
	while (njob->state == FG && njob != NULL)	 {sleep(0);}	
